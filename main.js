...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (missionsGrid) {
        ...
        const MISSIONS = [
            ...
            { titre: 'Gestion de tickets IT — DGFIP', desc: 'Traitement des demandes via le workflow CDS → DDFIP → AP Assist → CID. Réception, analyse et priorisation des tickets selon leur périmètre (informatique ou non). Suivi des interventions et communication avec les agents.', tag: 'stage', cat: 'incidents', ctx: 'Stage DGFIP — ESI Versailles' },
            { titre: 'Interventions postes — BitLocker & SecureBoot', desc: 'Extraction des informations logicielles des postes clients. Gestion de BitLocker via manage-bde (activation, désactivation, récupération de clé). Vérification du SecureBoot UEFI sur les machines en environnement DSI publique.', tag: 'stage', cat: 'patrimoine', ctx: 'Stage DGFIP — ESI Versailles' },
            { titre: 'Inventaire du parc informatique', desc: 'Utilisation du robot d\'inventaire Sigma (basé sur GLPI) pour recenser et mettre à jour les informations des postes (numéro, état, date). Gestion des comptes associés dans l\'Active Directory, pluralité des profils utilisateurs sur un même poste.', tag: 'stage', cat: 'patrimoine', ctx: 'Stage DGFIP — ESI Versailles' },
            { titre: 'Infrastructure réseau — Baies et câblage', desc: 'Identification de l\'architecture réseau sur site : baies de brassage, câbles VD/DP et VGA, switches d\'accès. Chaque poste dispose d\'une adresse IP liée au switch afin d\'être identifié et localisé. Configuration des connexions proxy et changement de serveur.', tag: 'stage', cat: 'patrimoine', ctx: 'Stage DGFIP — ESI Versailles' },
        ];
