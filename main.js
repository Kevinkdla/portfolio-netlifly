...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (missionsGrid) {
        ...
        const MISSIONS = [
            ...
            { titre: 'Gestion de tickets IT — Omega', desc: 'Traitement des demandes d\'intervention via l\'outil de ticketing Omega. Réception, analyse et priorisation des tickets selon leur périmètre (matériel, logiciel, fiabilisation). Suivi des interventions et retour aux agents sur les sites des Yvelines gérés par la CID.', logo: 'images/logo-dgfip.png', tag: 'stage', cat: 'incidents', ctx: 'Stage DGFIP — CID Versailles V12', tech: ['Omega', 'ITSM', 'ticketing'] },
            { titre: 'Campagnes de fiabilisation & interventions postes', desc: 'Participation aux campagnes de fiabilisation du parc : vérification BitLocker (manage-bde), contrôle SecureBoot UEFI, extraction d\'inventaire logiciel. Les postes tournent sous Windows 11 socle, image OS durcie selon les recommandations ANSSI : télémétrie désactivée, configuration sécurisée pour garantir la souveraineté numérique.', logo: 'images/logo-dgfip.png', tag: 'stage', cat: 'patrimoine', ctx: 'Stage DGFIP — CID Versailles V12', tech: ['BitLocker', 'manage-bde', 'SecureBoot', 'Windows 11'] },
            { titre: 'Inventaire du parc — Sigma / GLPI', desc: 'Utilisation du robot d\'inventaire Sigma (basé sur GLPI) pour recenser et mettre à jour les informations des postes agents (numéro, état, date d\'inventaire physique). Gestion des comptes associés dans l\'Active Directory et suivi de la pluralité des profils utilisateurs.', logo: 'images/logo-dgfip.png', tag: 'stage', cat: 'patrimoine', ctx: 'Stage DGFIP — CID Versailles V12', tech: ['GLPI', 'Sigma', 'Active Directory'] },
            { titre: 'Architecture réseau — RIE & infrastructure site', img: 'images/IMG_4188.jpeg', logo: 'images/logo-dgfip.png', desc: 'Découverte de l\'infrastructure réseau DGFIP : RIE (Réseau Interministériel de l\'État) réseau privé commun à tous les ministères, accès internet via la PFAI (plateforme proxy centralisée) — le VPN est réservé au télétravail. Identification des LTI (Local Technique Immeuble) et LTE (Local Technique d\'Étage) gérés par le service SYL, câblage VD/DP, baies de brassage et adressage IP par switch.', tag: 'stage', cat: 'patrimoine', ctx: 'Stage DDFIP — Baie de brassage V12', tech: ['RIE', 'PFAI', 'VLAN', 'LTI/LTE'] },
            { titre: 'Installation et mise en service des postes agents', img: 'images/IMG_4191.jpeg', desc: 'Installation complète du matériel (dock, câble Thunderbolt, écrans via HDMI/VGA) et déploiement du socle Windows 11 DGFIP via clé de soclage (Lite Touch Installation). Configuration BIOS Secure Boot, choix du département et du numéro d\'inventaire, activation du mode itinérance. Post-soclage : BitLocker, certificats VPN Cisco, imprimantes, macros LibreOffice.', logo: 'images/logo-dgfip.png', tag: 'stage', cat: 'patrimoine', ctx: 'Stage DGFIP — CID Versailles V12', tech: ['MDT', 'LTI', 'Windows 11 Socle', 'BitLocker', 'SecureBoot'] },
            { titre: 'Intervention sur site — Serveur DDFIP', img: 'images/IMG_4192.jpeg', logo: 'images/logo-dgfip.png', desc: 'Déplacement sur le site de Saint-Germain-en-Laye dans le cadre des interventions terrain du CID. Les agents du CID sont itinérants et disposent d\'un bureau sur chaque site. Découverte du serveur local de la DDFIP 78 hébergeant les services locaux du site.', tag: 'stage', cat: 'patrimoine', ctx: 'Stage DDFIP — Site Saint-Germain-en-Laye', tech: ['terrain', 'serveur', 'DDFIP 78'] },
            { titre: 'Préparation des accès agents — Samba4 (BaoS) & Agenti', desc: 'Configuration des comptes en vue de l\'arrivée de nouveaux agents. Affectation au GPO du site de travail via BaoS (outil de gestion des GPO s\'appuyant sur Samba4, implémentation open source d\'Active Directory). Placement dans les répertoires partagés correspondant au service métier via Agenti (ex : Trésorerie Hospitalière).', logo: 'images/logo-dgfip.png', tag: 'stage', cat: 'patrimoine', ctx: 'Stage DGFIP — CID Versailles V12', tech: ['BaoS', 'Samba4', 'GPO', 'Agenti', 'Active Directory'] },
        ];
            ...
            MISSIONS.forEach(m => {
                ...
                html += `
                    <div class="mission-card">
                        ${m.img ? `<img src="${m.img}" alt="${m.titre}" class="mission-card__img" loading="lazy">` : ''}
                        <div class="mission-card__head">
                            <h4 class="mission-card__title">${m.titre}</h4>
                            <div style="display:flex;align-items:center;gap:8px;flex-shrink:0;">
                                ${m.logo ? `<img src="${m.logo}" alt="" class="mission-card__logo" loading="lazy">` : ''}
                        ...
                        ${m.tech && m.tech.length ? `<div class="mission-card__techs">${m.tech.map(t => `<span class="mission-card__tech">${t}</span>`).join('')}</div>` : ''}
                    </div>`;
            });
